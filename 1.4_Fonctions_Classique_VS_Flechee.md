...
const obj = {
  name: 'MonObjet',
  classic: function() {
    return `Classique: ${this.name}`; // ✅ this = obj
  },
  arrow: () => {
    return `Fléchée: ${this.name}`;   // ❌ this = undefined (en strict mode)
  }
}
````
Cas React : 
...
const flechee = (...args) => {
  console.log(args);     // Tableau
};
````

````jsx
const List = ({ items }) => {
  // Conversion en tableau obligatoire pour les fléchées
  const logItems = (...items) => {
    console.log(items); // [{...}, {...}]
  };

  return <button onClick={() => logItems(items)}>Logger</button>;
};
````
...
```
Les fonctions fléchées sont aussi limitées, comme par exemple

````js
function User(name) { this.name = name; } // ✅ Constructeur
const UserArrow = (name) => { this.name = name; }; // ❌ Erreur
````

Equivalent en React

````jsx
// Fonction classique (hoistée)
render() {
...
// Fonction fléchée (non hoistée)
const sayHi = () => "Salut"; // Doit être déclarée avant usage
````
